...
public class AtunModel<T extends AtunEntity> extends HierarchicalModel<T> {
    public static final ModelLayerLocation LAYER_LOCATION =
            new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(PeruviansDelight.MOD_ID, "atun"), "main");
    ...
    @Override
    public void setupAnim(AtunEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose);
        if(!entity.isInWater()) {
            this.animate(entity.idleAnimationState, AtunAnimations.ATUN_FUERA, ageInTicks, 1f);
        }
        else {
            this.animate(entity.idleAnimationState, AtunAnimations.ATUN_IDLE, ageInTicks, 1f);
        }
    }

    @Override
    public void renderToBuffer(PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay, int color) {
        atun.render(poseStack, vertexConsumer, packedLight, packedOverlay, color);
    }
